function [cineq, ceq] = parameter_search_base(terminal_search)
...
% Set equality constraint to be M(t0) = initial CO2, T(t0) = initial temp
ceq = [y(end,3) - params.initial_temp];

% Return error in initial temperature level
if ~isempty(ceq)
    ...
    % Sometimes an initial guess results in CO2 monotonically increasing in
    % reverse time (doesn't bend back down): set large error
